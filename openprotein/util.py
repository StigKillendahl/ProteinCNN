...
def contruct_dataloader_from_disk(filename, minibatch_size, use_evolutionary=True, with_id=False):
    if with_id:
        return torch.utils.data.DataLoader(H5PytorchDatasetWithID(filename), batch_size=minibatch_size,
                                       shuffle=False, collate_fn=H5PytorchDatasetWithEvo.merge_samples_to_minibatch)
    elif use_evolutionary:
        return torch.utils.data.DataLoader(H5PytorchDatasetWithEvo(filename), batch_size=minibatch_size,
                                       shuffle=True, collate_fn=H5PytorchDatasetWithEvo.merge_samples_to_minibatch)
    else:
...
class H5PytorchDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        seq_length = self.h5pyfile['seq_length'][index][0]
        mask = torch.Tensor(self.h5pyfile['mask'][index,:seq_length]).type(dtype=torch.uint8)
        prim = torch.Tensor(self.h5pyfile['primary'][index,:seq_length]).type(dtype=torch.long)
        tertiary = torch.Tensor(self.h5pyfile['tertiary'][index][:seq_length]) # max length x 9
...
class H5PytorchDatasetWithEvo(H5PytorchDataset):

    def __init__(self, filename):
        super(H5PytorchDatasetWithEvo, self).__init__(filename)
        
    def __getitem__(self, index):
        seq_length = self.h5pyfile['seq_length'][index][0]
        mask = torch.Tensor(self.h5pyfile['mask'][index,:seq_length]).type(dtype=torch.uint8)
        p_id = self.h5pyfile['id'][index]
        prim = torch.Tensor(self.h5pyfile['primary'][index,:seq_length]).type(dtype=torch.long)
        tertiary = torch.Tensor(self.h5pyfile['tertiary'][index][:seq_length]) # max length x 9
        evolutionary = torch.Tensor(self.h5pyfile['evolutionary'][index][:seq_length]) # max length x 21
        return  prim, tertiary, mask, p_id, evolutionary

class H5PytorchDatasetWithID(H5PytorchDataset):
    
    def __init__(self, filename):
        super(H5PytorchDatasetWithID, self).__init__(filename)
    ...
    def __getitem__(self, index):
        ...
        tertiary = torch.Tensor(self.h5pyfile['tertiary'][index][:seq_length]) # max length x 9
        p_id = self.h5pyfile['id'][index]
        return  prim, tertiary, mask, p_id


def set_experiment_id(data_set_identifier, learning_rate, minibatch_size, run_name=""):
    output_string = run_name + "_"
...
def evaluate_model(data_loader, model, use_gpu, loss_atoms, use_evolutionary):
    loss = 0
    data_total = []
    dRMSD_list = []
    RMSD_list = []
    for i, data in enumerate(data_loader, 0):
        data = list(data)
        primary_sequence, tertiary_positions, mask = data[:3]
        ...
        start = time.time()
        predicted_angles, backbone_atoms, batch_sizes = model((amino_acids, batch_sizes))
        ...
        start = time.time()
        for primary_sequence, tertiary_positions, predicted_pos, predicted_backbone_atoms, mask in minibatch_data:
            tertiary_positions = apply_mask(tertiary_positions,mask)
            predicted_backbone_atoms = apply_mask(predicted_backbone_atoms[:len(primary_sequence)], mask)
            actual_coords = tertiary_positions.contiguous().view(-1,3)
            predicted_coords = predicted_backbone_atoms.contiguous().view(-1,3).detach()

            rmsd = calc_rmsd(predicted_coords, actual_coords)
            drmsd = calc_drmsd(predicted_coords, actual_coords, loss_atoms)
...
def write_model_to_disk(model, model_type="val"):
    path = "output/models/"+globals().get("experiment_id")+ "_" + model_type +".model"
    torch.save(model,path)
    return path


def load_model_from_disk(model_path, use_gpu):
    if use_gpu:
...
def write_run_to_disk(run):
    path = "output/runs/"+globals().get("experiment_id")+".p"
...
def calculate_dihedral_angles_over_minibatch(atomic_coords_padded, batch_sizes, use_gpu):
    ...
    for idx, _ in enumerate(batch_sizes):
        a_c = atomic_coords[idx][:batch_sizes[idx]]
       # a_c = apply_mask(a_c, masks[idx], size=9)
        angles.append(calculate_dihedral_angels(a_c, use_gpu))
...
def calc_drmsd(chain_a, chain_b, loss_atoms, use_gpu=False):
    if loss_atoms == 'c_alpha':
        chain_a = chain_a[1::3]
        chain_b = chain_b[1::3]
...
def calc_angular_difference(a1, a2, loss_atoms='all'):
...
def calc_avg_drmsd_over_minibatch(backbone_atoms_padded, actual_coords_padded, batch_sizes, use_gpu, loss_atoms='all'):
    ...
    for idx, backbone_atoms in enumerate(backbone_atoms_list):
        ...
        actual_coords = actual_coords_list[idx].contiguous().view(-1, 3)
        backbone_atoms = backbone_atoms.contiguous().view(-1,3)

        if use_gpu:
            actual_coords = actual_coords.cuda()
            backbone_atoms = backbone_atoms.cuda()

        drmsd_avg += calc_drmsd(backbone_atoms, actual_coords, loss_atoms, use_gpu=use_gpu)
