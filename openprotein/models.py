...
class ExampleModel(nn.Module):
    def __init__(self, embedding_size, minibatch_size, use_gpu, dropout=[0.0,0.0], mixture_size=500, hidden_size=25):
        super(ExampleModel, self).__init__()
        self.input_dropout = dropout[0] > 0.0
        self.hidden_size = hidden_size
        self.use_gpu = use_gpu
        self.num_lstm_layers = 2
        self.mixture_size = mixture_size
        self.bi_lstm = nn.LSTM(embedding_size, self.hidden_size,
                               num_layers=self.num_lstm_layers, bidirectional=True, bias=True, dropout=dropout[0])
    ...
    def _get_network_emissions(self, amino_acids):
        ...
        packed_input_sequences = rnn_utils.pack_padded_sequence(data, batch_sizes)
...
class soft_to_angle(nn.Module):
    def __init__(self, mixture_size):
        ...
        if mixture_size in [50,100,500]:
            ...
            phi_components = np.random.uniform(-math.pi, math.pi, mixture_size)
            psi_components = np.random.uniform(-math.pi, math.pi, mixture_size)
